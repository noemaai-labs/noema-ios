...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_noema_moe_weighted_reduce(
        ggml_metal_library_t lib) {
    const char * name = "kernel_noema_moe_weighted_reduce";
    ggml_metal_pipeline_with_params res = ggml_metal_library_get_pipeline(lib, name);
    if (!res.pipeline) {
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_memset(ggml_metal_library_t lib, const ggml_tensor *  op) {
    ...
    // This kernel writes 64-bit zero words and is also used to clear sparse
    // F32 operation outputs before active rows are evaluated.
    snprintf(base, 256, "kernel_memset_i64");
    snprintf(name, 256, "%s", base);
