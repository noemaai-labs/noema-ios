...
kernel void kernel_noema_moe_weighted_reduce(
        constant ggml_metal_kargs_noema_moe_weighted_reduce & args,
        device const char * experts,
        device const char * weights,
        device       char * dst,
        uint gid[[thread_position_in_grid]]) {
    const uint64_t total = (uint64_t) args.d * (uint64_t) args.t;
    if ((uint64_t) gid >= total) {
        return;
    }
    const int64_t id = gid % args.d;
    const int64_t it = gid / args.d;
    float sum = 0.0f;
    // Fixed logical top-k order gives deterministic accumulation and avoids
    // atomics even when several assignments select the same physical slot.
    for (int64_t ik = 0; ik < args.k; ++ik) {
        const device float * v = (const device float *) (experts +
            id * args.experts_nb0 + ik * args.experts_nb1 + it * args.experts_nb2);
        const device float * w = (const device float *) (weights +
            ik * args.weights_nb1 + it * args.weights_nb2);
        // Keep multiplication and accumulation in separate statements so
        // the result matches the legacy MUL then ordered ADD graph instead
        // of contracting into an FMA.
        const float weighted = *v * *w;
        sum = ik == 0 ? weighted : sum + weighted;
    }
    *(device float *) (dst + id * args.dst_nb0 + it * args.dst_nb1) = sum;
}
...
kernel void kernel_mul_mv_id(
    ...
    // Expert-major branches mark assignments from other groups inactive.
    // The encoder pre-zeroes their rows, so no placeholder expert work is
    // needed here.
    if (i02 < 0) {
        return;
    }
