...
void ggml_mul_mat_id_set_skip_inactive(struct ggml_tensor * a, bool skip_inactive) {
    GGML_ASSERT(a != NULL && a->op == GGML_OP_MUL_MAT_ID);
    ggml_set_op_params_i32(a, 0, skip_inactive ? 1 : 0);
}

bool ggml_mul_mat_id_get_skip_inactive(const struct ggml_tensor * a) {
    GGML_ASSERT(a != NULL && a->op == GGML_OP_MUL_MAT_ID);
    return ggml_get_op_params_i32(a, 0) != 0;
}

struct ggml_tensor * ggml_noema_moe_weighted_reduce(
        struct ggml_context * ctx,
        struct ggml_tensor  * experts,
        struct ggml_tensor  * weights) {
    GGML_ASSERT(experts != NULL && weights != NULL);
    ...
    const int64_t ne[2] = { experts->ne[0], experts->ne[2] };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 2, ne);
    result->op = GGML_OP_NOEMA_MOE_WEIGHTED_REDUCE;
