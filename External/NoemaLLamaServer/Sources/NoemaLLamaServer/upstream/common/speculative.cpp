...
static bool common_speculative_mtp_pair_is_compatible(
        const llama_model * model_tgt,
        const llama_model * model_dft) {
    if (model_tgt == nullptr || model_tgt->hparams.n_layer_nextn <= 0) {
        SPC_WRN("%s", "MTP sidecars require a target model that declares NextN output layers\n");
        return false;
    }
    if (model_tgt->hparams.n_layer_nextn != model_dft->hparams.n_layer_nextn) {
        SPC_WRN("MTP sidecar NextN layer count %d does not match target declaration %d\n",
                model_dft->hparams.n_layer_nextn, model_tgt->hparams.n_layer_nextn);
        return false;
    }
    if (!common_speculative_are_compatible(model_tgt, model_dft)) {
        return false;
    }
    ...
    if (n_vocab_tgt != n_vocab_dft) {
        SPC_WRN("MTP sidecar vocab size %d does not match target vocab size %d\n", n_vocab_dft, n_vocab_tgt);
        return false;
    }

    // MTP predicts target token IDs directly, so unlike a generic helper draft
    // every ID (including the low special-token range) must map identically.
    for (int i = 0; i < n_vocab_tgt; ++i) {
        if (std::strcmp(llama_vocab_get_text(vocab_tgt, i), llama_vocab_get_text(vocab_dft, i)) != 0) {
            SPC_WRN("MTP sidecar token %d does not match the target vocabulary\n", i);
            return false;
        }
    }
...
static bool common_speculative_model_supports_mtp(const llama_model * model) {
    if (model == nullptr || model->hparams.n_layer_nextn <= 0) {
        SPC_WRN("%s", "MTP requires at least one declared NextN layer\n");
        return false;
    }
    ...
    switch (model->arch) {
        ...
        case LLM_ARCH_STEP35:
            break;
        default:
            SPC_WRN("MTP graph execution is not implemented for architecture '%s'\n", model->arch_name().c_str());
            return false;
    }

    const int32_t first = model->hparams.n_layer();
    const int32_t end   = first + model->hparams.n_layer_nextn;
    if (first < 0 || end > (int32_t) model->layers.size()) {
        SPC_WRN("MTP layer range [%d, %d) is outside the loaded layer set (%zu)\n", first, end, model->layers.size());
        return false;
    }

    for (int32_t il = first; il < end; ++il) {
        const auto & nextn = model->layers[il].nextn;
        if (nextn.eh_proj == nullptr || nextn.enorm == nullptr || nextn.hnorm == nullptr) {
            SPC_WRN("MTP layer %d is missing required NextN projection or normalization tensors\n", il);
            return false;
        }
    }
...
static float common_speculative_full_vocab_max_probability(llama_context * ctx, int32_t idx) {
    ...
    for (int32_t token = 0; token < n_vocab; ++token) {
        denominator += std::exp((double) logits[token] - max_logit);
    }
    return denominator > 0.0 ? (float) (1.0 / denominator) : 0.0f;
}
...
int32_t common_speculative_effective_mtp_cap(
        ...
        bool chain_heads) {
    int32_t result = std::max(0, configured_n_max);
    if (round_n_max > 0) {
        result = std::min(result, round_n_max);
    }
    if (chain_heads) {
        result = std::min(result, std::max(0, n_mtp_layers));
    }
    return result;
}
...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
        ...
        , params(params.draft)
    {
        ...
        if (n_embd != llama_model_n_embd(llama_get_model(ctx_tgt))) {
            throw std::runtime_error("MTP hidden width does not match the target model");
        }
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        int n_drafting = 0;
        std::vector<bool> drafting(n_seq);
        std::vector<int32_t> effective_n_max(n_seq, 0);
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            const int32_t cap = common_speculative_effective_mtp_cap(
                    params.n_max, dp.n_max, n_mtp_layers, chain_heads);
            if (cap <= 0) {
                continue;
            }
        ...
        while (n_drafting > 0) {
            ...
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                ...
                const float full_vocab_p = common_speculative_full_vocab_max_probability(ctx_dft, i_last[seq_id]);
                ...
                if (effective_n_max[seq_id] <= (int) result.size()) {
                    drafting[seq_id] = false;
                    n_drafting--;
                    continue;
                }
    ...
    bool get_state(llama_seq_id seq_id, std::vector<uint8_t> & data) const override {
        ...
        static constexpr uint32_t state_magic = 0x4d545031; // "MTP1"
        const size_t row_bytes = (size_t) n_embd * sizeof(float);
        data.resize(sizeof(state_magic) + sizeof(n_embd) + row_bytes);
        std::memcpy(data.data(), &state_magic, sizeof(state_magic));
        std::memcpy(data.data() + sizeof(state_magic), &n_embd, sizeof(n_embd));
        std::memcpy(data.data() + sizeof(state_magic) + sizeof(n_embd), pending_h[seq_id].data(), row_bytes);
        return true;
    }
    ...
    void set_state(llama_seq_id seq_id, const std::vector<uint8_t> & data) override {
        ...
        static constexpr uint32_t expected_magic = 0x4d545031;
        const size_t row_bytes = (size_t) n_embd * sizeof(float);
        if (data.size() != sizeof(expected_magic) + sizeof(n_embd) + row_bytes) {
            SPC_WRN("invalid MTP state size for seq_id=%d: got %zu\n", (int) seq_id, data.size());
            reset();
            return;
        }

        uint32_t magic = 0;
        int32_t saved_n_embd = 0;
        std::memcpy(&magic, data.data(), sizeof(magic));
        std::memcpy(&saved_n_embd, data.data() + sizeof(magic), sizeof(saved_n_embd));
        if (magic != expected_magic || saved_n_embd != n_embd) {
            SPC_WRN("incompatible MTP state for seq_id=%d\n", (int) seq_id);
            reset();
            return;
        }

        std::memcpy(pending_h[seq_id].data(), data.data() + sizeof(magic) + sizeof(saved_n_embd), row_bytes);
        verify_h[seq_id].clear();
        verify_h_rows[seq_id] = 0;
        i_last[seq_id] = -1;
        if (chain_heads) {
...
common_speculative_init_result::common_speculative_init_result(
    ...
    if (spec_mtp) {
        ...
        // The MTP context uses the target model's dense-attention head and
        // therefore has no recurrent state to snapshot independently.
        cparams.n_rs_seq = 0;
    }

    // Helper-draft contexts mirror target verification batches. Preserve the
    // rollback window requested by common_context_params_to_llama so hybrid
    // Qwen helpers can discard rejected suffixes without an unsupported
    // partial recurrent-state removal. Unsupported architectures clamp this
    // value to zero in llama_context and keep their existing fallback.
    ...
    if (has_draft) {
        ...
        if (spec_mtp) {
            if (!common_speculative_model_supports_mtp(model_dft)) {
                LOG_ERR("%s: draft model does not contain an executable MTP graph\n", __func__);
                pimpl->model.reset();
                return;
            }
            if (model_dft->arch != model_tgt->arch ||
                llama_model_n_embd(model_dft) != llama_model_n_embd(model_tgt) ||
                !common_speculative_mtp_pair_is_compatible(model_tgt, model_dft)) {
                LOG_ERR("%s: MTP sidecar is incompatible with the target model\n", __func__);
                pimpl->model.reset();
                return;
            }
        }
        ...
        if (!common_speculative_model_supports_mtp(model_tgt)) {
            LOG_ERR("%s: target model does not contain an executable embedded MTP graph\n", __func__);
            return;
        }
