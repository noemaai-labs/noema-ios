...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--spec-draft-dynamic"},
        {"--no-spec-draft-dynamic"},
        string_format("adapt the draft length at runtime from the measured acceptance rate and throughput (default: %s)",
                      params.speculative.draft.dynamic ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.speculative.draft.dynamic = value;
        }
    ).set_spec().set_examples({LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}).set_env("LLAMA_ARG_SPEC_DRAFT_DYNAMIC"));
    add_opt(common_arg(
