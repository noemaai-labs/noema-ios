...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    // Noema Overfit: cross-check bank coverage against the paged manifest and
    // preload sidecar expert records (never reached on the no-alloc sizing
    // path, which returns above).
    if (noema_paged_active()) {
        const char * paged_err = nullptr;
        if (!noema_paged_finalize_load(llm_arch_name(arch), &paged_err)) {
            throw std::runtime_error(format("noema_paged: %s", paged_err ? paged_err : "finalize failed"));
        }
    }
