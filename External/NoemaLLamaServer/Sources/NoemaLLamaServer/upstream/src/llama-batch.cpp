...
llama_ubatch llama_batch_allocr::split_equal(uint32_t n_ubatch, bool sequential, uint32_t n_keep_tail) {
    ...
    if (n_keep_tail > 0) {
        // Equality is valid: a final rollback window can occupy the whole
        // ubatch. If more tokens remain, the logic below either leaves at
        // least n_keep_tail for the next split or emits the sequence through
        // completion. Requiring one extra token made a correctly bounded
        // N + 1 speculative verification batch abort at context startup.
