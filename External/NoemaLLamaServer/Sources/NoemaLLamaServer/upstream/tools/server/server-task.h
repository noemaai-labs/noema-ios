...
struct task_params {
    ...
    bool allow_speculative = true; // allow the server's configured speculative decoder for this request
...
struct result_timings {
    ...
    // Optional speculative metrics. Attempts are emitted even when no token was proposed.
    std::string speculative_type;
    std::string speculative_state;
    int32_t draft_attempts = 0;
    int32_t draft_empty_attempts = 0;
    int32_t draft_n = 0;
    int32_t draft_n_accepted = 0;
    int32_t draft_n_budget = -1;
    double draft_ms = 0.0;
    double draft_verification_ms = 0.0;
    double draft_rollback_ms = 0.0;
    std::vector<int32_t> draft_accepted_per_position;

    // current dynamic draft length; -1 = dynamic drafting off, 0 = suspended
    int32_t draft_n_dyn = -1;
