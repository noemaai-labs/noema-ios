...
final class CoreAIPipelinedEngine: InferenceEngine, Sendable {
    ...
    func generate(
        ...
        Task {
            // Bounded wait instead of the trapping acquireEngine(): a consumer
            // that stopped iterating may leave the previous producer winding
            // down for a few steps; surface an error rather than crash if the
            // engine genuinely never comes back.
            var acquireAttempts = 0
            while !self.tryAcquireEngine() {
                acquireAttempts += 1
                if acquireAttempts > 15000 {
                    inferenceStream.setStopReason(.error)
                    outputContinuation.finish(
                        throwing: InferenceRuntimeError.invalidState(
                            "Engine still held by a previous generation after 15s."))
                    return
                }
            ...
            do {
                ...
                // NOEMA PATCH: when the consumer stops consuming the outer
                // stream (early EOS break, Stop, or deallocation), finish the
                // inner stream so runCompletion's isCancelled flips and the
                // GPU loop exits promptly. Upstream only stops at maxTokens,
                // which leaves the engine held for the whole remaining context
                // and makes the next reset()'s drain() trap.
    ...
    private func drain() {
        ...
        while engineInUse.load(ordering: .acquiring) {
            attempts += 1
            if attempts > 15000 {
                // NOEMA PATCH: don't crash the app — skip the reset (the
                // tryAcquireEngine() guard in reset() bails) and let the next
                // generate() surface an error if the engine never returns.
                CLILogger.log(
                    "CoreAIPipelinedEngine.drain(): engine not returned after 15s; skipping reset")
                return
            }
